...
//            "where  " +
//            "a2.rn >= #{start_page}")
    @Select("select id, user_name as userName, add_time from user_info where user_name like concat('%', concat(#{keyword}, '%')) and id != #{user_id} and id not in (select id from user_info where id in (select b_id from friends where a_id = #{user_id} and status = #{status} union " +
            "select a_id from friends where b_id = #{user_id} and status = #{status}) ) order by id desc limit #{start_page}, #{page_size}")
    //@Select("select * from user_info")
    List<User> findUserByUserName(@Param("start_page") int start_page, @Param("page_size") int page_size, @Param("user_id") int user_id, @Param("keyword") String userName, @Param("status") int status);
     ...
     */
    @Transactional(readOnly = true)
    @Select("select count(id) from user_info where user_name like concat('%', concat(#{keyword}, '%')) and id != #{user_id} and id not in (select id from user_info where id in (select b_id from friends where a_id = #{user_id} and status = #{status} union "  +
            "select a_id from friends where b_id = #{user_id} and status = #{status}))")
